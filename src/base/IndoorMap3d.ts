...
export class IndoorMap3d {
	...
	init(mall: Mall) {
		...
		this.rootEle.style.background = this.theme.background;
	...
	setDefaultView() {
		...
		this.camera.position.set(camDir[1] * camLen, Math.sin(tiltAngle) * camLen, camDir[0] * camLen); //TODO: adjust the position automatically
	...
	onSelectObject(event: Event) {
		...
		let intersects = this.rayCaster.intersectObjects(this.mall.children[0].children);
	...
	// 更新商店名称、公共设施
	updateLabels() {
		if (this.mall == null || this.controls == null || !this.viewChanged) {
			return;
		}
		...
		if (this.showNames) {
			if (curFloor.nameSprites != undefined) {
				...
				ParseModel.updateSprites(curFloor.nameSprites, projectMatrix, [this.canvasWidthHalf, this.canvasHeightHalf]);
			}
		...
		if (this.isShowPubPoints) {
			if (curFloor.pubPointSprites != undefined) {
				...
				ParseModel.updateSprites(curFloor.pubPointSprites, projectMatrix, [this.canvasWidthHalf, this.canvasHeightHalf]);
			}
		}
		this.viewChanged = false;
	};
